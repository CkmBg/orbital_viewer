...
export default function OrbitVisualizer({
  ...
  const prevIdxRef = useRef(indexTransition);
  useEffect(() => {
    ...
    const diffIdx = indexTransition - prevIdxRef?.current <= 0;
    const range = planetsPoints?.map((elm) =>
      elm?.samples?.slice(
        diffIdx ? indexTransition : prevIdxRef?.current,
        diffIdx ? prevIdxRef?.current : indexTransition
      )
    );
    console.log(
      diffIdx,
    ...
    planetsRef.current?.map((elm, idx) =>
      RotatePlanet(
        elm,
        diffIdx ? range[idx]?.reverse() : range[idx],
        renderer,
        camera,
        scene,
        prevIdxRef,
  ...
  }, [trajectory, reset, planetsPoints, indexTransition]);
