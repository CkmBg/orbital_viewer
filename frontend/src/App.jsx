...
function App() {
  ...
  const loadAllTrajectories = async () => {
    if (!planetsOrbitalElements) {
      console.warn(
        "[App] Attempted to load trajectories before planetsOrbitalElements ready"
      );
      return;
    }
      ...
      for (const [planet, elems] of planetEntries) {
        if (elems && !elems.error) {
          ...
          try {
            ...
            console.log(
              `[App] Success trajectory planet=${planet} points=${orbitRes.trajectory?.length}`
            );
          } catch (innerErr) {
            console.error(
              `[App] Failed trajectory planet=${planet}:`,
              innerErr
            );
          ...
          if (elems?.error) {
            console.warn(
              `[App] Skipping planet=${planet} due to previous error: ${elems.error}`
            );
          } else {
  ...
  return (
    <div id="root">
      <VerticalMenu>
        <button
          onClick={loadAllTrajectories}
          style={{ marginTop: "20px" }}
          disabled={loadingTrajectories || !planetsOrbitalElements}
        >
      ...
      <div className="main-content">
        {Object.keys(trajectories).length > 0 ? (
          ...
          <div className="placeholder-message">
            {loadingPlanets
              ? "Loading planetary orbital elements..."
              : 'Click "Load Orbits" to fetch trajectories'}
          </div>
        )}
      </div>
    </div>
